...
const BASIS_NOTE: Record<Basis, string> = {
  ...
  unresolved: "no source for this — an engineer has to supply it",
};
...
function evidenceList(step: { evidence: { file: string; line: number }[] }): string {
  if (!step.evidence.length) return "—";
...
export function toMarkdown(draft: Draft): string {
  ...
  lines.push("");
  lines.push(`Generated ${date} by FixtureForge. **This is a draft, not a release.**`);
  lines.push("");
  lines.push(
  ...
  if (draft.limits.length) {
    ...
    for (const limit of draft.limits) {
      lines.push(
        `| ${limit.parameter} | ${limit.net ?? "—"} | ${formatLimit(limit)} | ${limit.basis} | ${evidenceList(limit)} |`,
      );
    }
  } else {
...
export function toPytest(draft: Draft): string {
  ...
  out.push("");
  out.push(`Generated by FixtureForge on ${new Date(draft.generatedAt).toISOString().slice(0, 10)}.`);
  out.push("");
